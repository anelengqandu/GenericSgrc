...
namespace Sgrc.Tests
{
    public abstract class SgrcTestBase : AbpIntegratedTestBase<SgrcTestModule>
    {
        ...
        protected void UsingDbContext(Action<SgrcDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SgrcDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SgrcDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SgrcDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SgrcDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SgrcDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SgrcDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SgrcDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SgrcDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SgrcDbContext, Task<T>> func)
        {
            T result;
